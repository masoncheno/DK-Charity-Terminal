...
function dashboard(){
 ...
 const openExposure=pending.reduce((a,b)=>a+Number(b.stake||0),0);
 const liveCount=state.games.filter(g=>g.status==="in").length;
 const todayGames=state.games.filter(g=>g.date && new Date(g.date).toDateString()===new Date().toDateString()).length;
 return `<div class="page-title">
  <div><div class="pill" style="margin-bottom:8px">${state.live?"● SHARED LIVE":"● LOCAL MODE"}</div><h1>Command Center</h1><p>Your shared betting dashboard — markets, games, bankroll and model tools.</p></div>
  <div style="display:flex;gap:8px;flex-wrap:wrap"><button class="ghost" id="dashRefresh">↻ Refresh</button><button class="primary" id="addBetBtn">+ Add Bet</button></div>
 </div>
 ...
 <div class="grid three" style="margin-top:14px">
  <section class="card"><div class="section-head"><h2>Live now</h2>${miniBadge(`${liveCount} games`)}</div><div class="hero"><div class="big">${liveCount}</div><small>Live games across connected feeds</small></div></section>
  <section class="card"><div class="section-head"><h2>Today's slate</h2>${miniBadge(`${todayGames} games`)}</div><div class="hero"><div class="big">${todayGames}</div><small>Scheduled games currently loaded</small></div></section>
  <section class="card"><div class="section-head"><h2>Open exposure</h2>${miniBadge(`${pending} pending`,"pending")}</div><div class="hero"><div class="big">${money(openExposure)}</div><small>Total stake currently unsettled</small></div></section>
 </div>
 <div class="grid two" style="margin-top:14px">
  <section class="card"><div class="section-head"><h2>Live / Upcoming</h2><span>Auto-refresh · ${Number(cfg.ESPN_REFRESH_MS||60000)/1000}s</span></div>${gamesList(7)}</section>
  <section class="card"><div class="section-head"><h2>Group leaderboard</h2><span>settled bets</span></div>${leaderboard()}<div style="margin-top:12px"><button class="ghost" data-go="analytics">Open analytics →</button></div></section>
 </div>
 <div class="grid two" style="margin-top:14px">
  ...
  <section class="card"><div class="section-head"><h2>Terminal status</h2></div>
   <div class="kpi"><span>Shared database</span><b class="${state.live?"pos":""}">${state.live?"CONNECTED":"LOCAL ONLY"}</b></div>
   <div class="kpi"><span>Sports feed</span><b class="${state.games.length?"pos":""}">${state.games.length?"LIVE":"WAITING"}</b></div>
   <div class="kpi"><span>Sports supported</span><b>${SPORTS.length}</b></div>
   <div class="kpi"><span>Last refresh</span><b>${new Date().toLocaleTimeString()}</b></div>
...
function gamesPage(){return `<div class="page-title"><div><div class="pill" style="margin-bottom:8px">ESPN PUBLIC FEEDS</div><h1>Games</h1><p>Live and scheduled games from the connected sports feed.</p></div><button class="ghost" id="refreshGames">↻ Refresh games</button></div><section class="card">${gamesList(100)}</section>`}
function livePage(){const live=state.games.filter(g=>g.status==="in");return `<div class="page-title"><div><div class="pill" style="margin-bottom:8px">LIVE SPORTS</div><h1>Live Terminal</h1><p>Live scores and market workspace.</p></div><div style="display:flex;gap:8px;align-items:center"><span class="pill">${live.length} LIVE</span><button class="ghost" id="liveRefresh">↻ Refresh</button></div></div><div class="grid three">${(live.length?live:state.games.slice(0,3)).map(g=>`<div class="hero"><small>${esc(g.league||"SPORT")} · ${esc(g.statusText||"")}</small><div class="big">${esc(g.awayScore??"-")} — ${esc(g.homeScore??"-")}</div><div>${esc(g.away)} @ ${esc(g.home)}</div><hr style="border-color:var(--border)"><div class="kpi"><span>Moneyline</span><b>${esc(g.awayOdds||"—")} / ${esc(g.homeOdds||"—")}</b></div><div class="kpi"><span>Spread</span><b>${esc(g.spread||"—")}</b></div><div class="kpi"><span>Total</span><b>${esc(g.total||"—")}</b></div></div>`).join("")||`<div class="empty">No live games currently available.</div>`}</div>`}
function analyticsPage(){
 const sports=[...new Set(state.bets.map(b=>b.sport))].filter(Boolean);
 return `<div class="page-title"><div><div class="pill" style="margin-bottom:8px">BANKROLL & PERFORMANCE</div><h1>Analytics</h1><p>Find where your process is actually working.</p></div><span class="pill">${state.bets.length} TOTAL BETS</span></div>
...
function bindPage(){\n $("#liveRefresh")?.addEventListener("click",loadGames);
 $("#addBetBtn")?.addEventListener("click",openBet); $("#dashRefresh")?.addEventListener("click",loadGames);
