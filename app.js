...
function gamesList(n){
 ...
 return state.games.slice(0,n).map(g=>`<div class="game"><div><div><span class="pill">${esc(g.league||"SPORT")}</span>${g.status==="in"?" <span class='live-badge'>LIVE</span>":""}</div><div class="teams">${esc(g.away)} @ ${esc(g.home)}</div><small>${esc(g.statusText||g.date||"")}</small></div><div class="score">${g.awayScore??"-"} — ${g.homeScore??"-"}</div></div>`).join("");
}
...
function betTable(bs){
 ...
 return `<div class="table-wrap"><table class="table"><thead><tr><th>Bettor</th><th>Game</th><th>Selection</th><th>Odds</th><th>Stake</th><th>Result</th><th>P/L</th></tr></thead><tbody>${bs.map(b=>`<tr><td>${esc(b.bettor)}</td><td>${esc(b.game)}</td><td>${esc(b.selection)}</td><td>${Number(b.odds)>0?"+":""}${esc(b.odds)}</td><td>${money(b.stake)}</td><td class="${b.result==="Win"?"pos":b.result==="Loss"?"neg":"pending"}">${esc(b.result)}</td><td class="${(pnl(b)||0)>=0?"pos":"neg"}">${pnl(b)===null?"—":money(pnl(b))}</td></tr>`).join("")}</tbody></table></div>`;
}
function betsPage(){
...
function bindPage(){
 ...
 $("#modelOdds")?.addEventListener("input",e=>$("#modelProb").textContent=pct(oddsToProb(e.target.value)));
...
async function loadGames(){
 try{
  const sports=[["football","nfl"],["basketball","nba"],["baseball","mlb"],["hockey","nhl"],["football","college-football"],["basketball","mens-college-basketball"],["soccer","eng.1"],["soccer","uefa.champions"]];
  const out=[];
  for(const [sport,league] of sports){
   try{
    ...
    for(const ev of (d.events||[]).slice(0,20)){
     ...
     out.push({id:ev.id,league:d.leagues?.[0]?.name||league,away:away?.team?.abbreviation||away?.team?.displayName||"Away",home:home?.team?.abbreviation||home?.team?.displayName||"Home",awayScore:away?.score??0,homeScore:home?.score??0,status:ev.status?.type?.state==="in"?"in":ev.status?.type?.state,statusText:ev.status?.type?.shortDetail||ev.status?.type?.description,date:ev.date});
    }
   }catch{}
  }
  state.games=out.sort((a,b)=>new Date(a.date)-new Date(b.date));render();
 }catch(e){console.warn(e)}
}
